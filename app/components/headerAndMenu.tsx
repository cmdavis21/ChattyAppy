...
import {
  ActiveChatProfileCard,
  ProfileContactCard,
  SwitchProfileCard,
} from "./chattProfileCards";

const AppHeaderAndMenu = () => {
  const [menuToggle, setMenuToggle] = useState(false);
  const [switchProfileMenu, setSwitchProfileMenu] = useState(false);
    ...
    <>
      ...
      <header className="sticky h-fit top-0 left-0 bg-transparent shadow-md rounded-b-2xl flex justify-between items-center mb-6 pt-[15px] px-[10px] lg:px-[5%] z-50">
        ...
        <div className="flex lg:absolute md:top-[40px] left-[35%] lg:top-[35px] lg:left-[29%] lg:bg-orange-800 rounded-full py-[8px] px-4 gap-4 items-center">
          <p className="hidden lg:block text-orange-200 text-[16px] italic">
            Your chattin&apos; with:
          </p>
          <GiPlagueDoctorProfile className="text-gray-800 lg:text-[#fffafa] text-[35px]" />
          <span className="text-gray-800 lg:text-orange-200 text-[16px]">
            Dr_bigBeak_004
          </span>
          <ActiveStatus />
        </div>
        ...
        <div className="hidden lg:flex gap-4 items-center min-w-[240px] max-w-[280px]">
          ...
          <span className="text-black text-[14px] break-words">
        ...
        {/* menu toggle button */}
        <button
          onClick={() => setMenuToggle((click) => !click)}
      ...
      <nav
        className={`${
          menuToggle ? "right-0" : "-right-[800px]"
        } transition-all ease-in duration-[600ms] bg-orange-800 opacity-85 overflow-y-scroll w-full md:w-[450px] absolute top-[68px] md:rounded-tl-[50px] md:rounded-bl-[50px] p-4 md:p-9 flex flex-col justify-between z-45`}
      >
        <div className="flex flex-col gap-y-8">
          ...
          <div>
            ...
            <div className="flex gap-4 items-center min-w-[240px] max-w-[280px]">
              ...
              <span
                onClick={() => setSwitchProfileMenu((click) => !click)}
                className="text-orange-200 text-[20px] hover:underline hover:underline-offset-1 break-words cursor-pointer"
              >
                Miss.Goaty.Girl!2010
              </span>
            </div>
          ...
          <div
            className={`${
              switchProfileMenu ? "block" : "hidden"
            } transition-[block] duration-800`}
          >
            ...
            <div className="bg-orange-900 p-5 h-[125px] w-[100%] overflow-y-scroll flex flex-col gap-y-2 rounded-xl">
              <SwitchProfileCard
                icon={<ImTux />}
                name="Mr_Flipper09"
                lastLogin="11/21/2023 12:06PM"
              />
              <SwitchProfileCard
                icon={<LuGhost />}
                name="xxSheAGhostGurlXx"
                lastLogin="04/24/2023 08:00PM"
              />
            </div>
          </div>

          {/* log off button */}
          <div className="flex gap-x-2 items-center w-[75%]">
            <button className="bg-transparent hover:bg-white rounded-full w-fit text-[20px] py-2 px-4 text-white hover:text-orange-900 peer">
              Log off
            </button>
            <div className="text-white pl-[5px] text-[11px] border-0 hidden peer-hover:block peer-hover:border peer-hover:border-x-0 peer-hover:border-t-0 peer-hover:border-b-[1px] peer-hover:border-b-white transition-[hidden, block, border] ease-in-out duration-900">
              we will miss you...
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              <ActiveChatProfileCard
                icon={<FaTheRedYeti />}
                name="immaYetiBB"
                recentMessage="Wanna get some snowcones? I was thinking that"
              />
              <ActiveChatProfileCard
                icon={<GiEgyptianProfile />}
                name="_daQUEENCleo_84"
                recentMessage="yurrrr, what up B?"
              />
              {/* create a new chatt */}
              <button className="bg-white rounded-full py-2 px-4 text-orange-900 hover:bg-orange-950 hover:text-white hover:cursor-pointer">
                + Create a new chatt
              </button>
            </div>
          </div>
